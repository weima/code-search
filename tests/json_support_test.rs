...
fn test_json_translation_support() {
    ...
    let query = SearchQuery::new("Save Changes".to_string())
        .with_base_dir(base_dir.clone());

    let result = run_search(query).expect("Search failed");
    
    // Verify translation entry found
    assert!(!result.translation_entries.is_empty(), "Should find translation entry");
    ...
    assert!(!result.code_references.is_empty(), "Should find code reference");
