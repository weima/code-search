...
fn test_search_finds_text_in_fixtures() {
    ...
    assert!(fixture_matches.len() > 0, "Should find at least one match for 'add new' in fixtures");

    // Should find in YAML file
    assert!(
        fixture_matches.iter().any(|m| m.file.to_str().unwrap().contains("en.yml")),
        "Should find match in YAML translation file"
    );

    // Should find in JSON files
    assert!(
        fixture_matches.iter().any(|m| m.file.to_str().unwrap().contains("en.json")),
...
fn test_search_case_insensitive() {
    ...
    assert!(fixture_matches.len() > 0, "Case-insensitive search should find matches");
}

#[test]
fn test_search_case_sensitive() {
    let searcher = TextSearcher::new().case_sensitive(true);
    ...
    assert_eq!(uppercase_fixtures.len(), 0, "Should not find 'ADD NEW' with case-sensitive search");
    assert!(lowercase_fixtures.len() > 0, "Should find 'add new' with case-sensitive search");
}
...
fn test_search_finds_function_names() {
    ...
    assert!(code_matches.len() >= 2, "Should find processPayment in multiple files");

    // Should find definition in utils.ts
    assert!(
        code_matches.iter().any(|m| m.file.to_str().unwrap().contains("utils.ts")),
        "Should find processPayment in utils.ts"
    );

    // Should find usage in checkout.ts
    assert!(
        code_matches.iter().any(|m| m.file.to_str().unwrap().contains("checkout.ts")),
...
fn test_search_finds_error_messages() {
    ...
    assert!(code_matches.len() >= 2, "Should find error message in multiple locations");
...
fn test_search_finds_variables() {
    ...
    assert!(code_matches.len() > 10, "Should find userId many times across files");
    ...
    assert!(
        code_matches.iter().any(|m| m.file.to_str().unwrap().contains("api.ts")),
...
fn test_search_no_matches() {
    ...
    let matches = searcher.search("xYzAbC987654321ThisShouldNeverExist").unwrap();
    ...
    assert_eq!(non_test_matches.len(), 0, "Should return empty vec when no matches found (excluding test file itself)");
}
...
fn test_match_includes_content() {
    ...
    for m in code_matches {
        ...
        assert!(m.content.contains("processPayment"), "Content should contain search text");
    }
}
